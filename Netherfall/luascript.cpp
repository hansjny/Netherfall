...
void LuaScriptInterface::registerFunctions()
{
	...
	//hauni dungeon generation
	lua_register(m_luaState, "doCreateNewDungeon", LuaScriptInterface::luaDoCreateNewDungeon);
	//getOTSYSTime()
	...
	lua_register(m_luaState, "getItemMagicAttributes", LuaScriptInterface::luaGetItemMagicAttributes);
	...
	//doPlayerAddItemWithAttribute(cid, itemid, attribute, weight)
	lua_register(m_luaState, "doPlayerAddItemWithAttribute", LuaScriptInterface::luaDoPlayerAddItemWithAttribute);
	...
	//doCoverGround(fromPos, toPos, itemid)
	lua_register(m_luaState, "doCoverGround", LuaScriptInterface::luaDoCoverGround);
	...
	//doItemAddAttribute(uid, attribute, weight)
	lua_register(m_luaState, "doItemAddAttribute", LuaScriptInterface::luaDoItemAddAttribute);
	...
	//hansjny / Netherfall Custom
	lua_register(m_luaState, "onCreatureDie", LuaScriptInterface::luaOnCreatureDie);
}
...
int LuaScriptInterface::luaDoItemAddAttribute(lua_State *L)
{
	int32_t parameters = lua_gettop(L);
	uint32_t weight = popNumber(L);
	uint32_t attribute = popNumber(L);
	...
	item->setMagAttr(attribute, weight);
	lua_pushboolean(L, true);
	return 1;
}

int LuaScriptInterface::luaDoPlayerAddItemWithAttribute(lua_State *L)
{
	//doPlayerAddItem(cid, itemid, attribute, weight)
	int32_t parameters = lua_gettop(L);
	uint32_t weight = popNumber(L);
	int32_t attribute = popNumber(L);
	int32_t itemId = (uint32_t)popNumber(L);
	...
	int32_t atk = it.attack;
	int32_t itemCount = 1;

	Item* newItem = Item::CreateItem(itemId, itemCount);
	
	newItem->setMagAttr(attribute, weight);
...
int LuaScriptInterface::luaGetItemMagicAttributes(lua_State *L) {
	int32_t parameters	= lua_gettop(L);
	uint32_t attrType 	= popNumber(L);
	uint32_t itemuid 	= popNumber(L);
	ScriptEnviroment* env = getScriptEnv();

	Item* item = env->getItemByUID(itemuid);
	int32_t attrval = item->getMagAttrWeight(attrType);
	if (attrval == 0) {
		lua_pushinteger(L, 0);
		return 0;
	}

	lua_pushinteger(L, attrval);
	return 1;
}
...
int LuaScriptInterface::luaDoCoverGround(lua_State *L)
{

	PositionEx startPos;
	popPosition(L, startPos);

	PositionEx endpos;
	popPosition(L, endpos);
	...
	for (int32_t i = startPos.x; i < endpos.x; i++)
	{
		for (int32_t j = startPos.y; j < endpos.y; j++) 
		{
			...
			tile = g_game.getMap()->getTile(i, j, startPos.z);

			if (tile == NULL) {
				tile = new StaticTile(i, j, startPos.z);
				g_game.getMap()->setTile(i, j, startPos.z, tile);
			}
...
int LuaScriptInterface::luaDoSummonCreature(lua_State *L)
{
	...
	std::cout << "Summoned: " << cid << std::endl;
	lua_pushnumber(L, cid);
	return 1;
}
...
static LuaStoredEventDesc* eventDesc = new LuaStoredEventDesc();
int LuaScriptInterface::luaOnCreatureDie(lua_State *L)
{
	// onCreatureDie(callback, creature, callbackParam1, ..., callbackParamN)
	ScriptEnviroment* env = getScriptEnv();
...
//	int parameter = luaL_ref(L, LUA_REGISTRYINDEX);
	if (!lua_istable(L, -1)) {
		reportError(__FUNCTION__, "third parmeter should be a table");
		lua_pushboolean(L, false);
		return 1;
	}
	...
	std::cout << script_interface->getFileById(env->getScriptId()) << std::endl;
	std::cout << "function: " << eventDesc->function << "\n";
	...
	//creature->registerCreatureEventCallback("popDungeon", eventDesc);
	creature->registerCreatureEvent("popDungeon");
	lua_pushboolean(L, true);
	return 1;
}
...
// hauni's dungeon generation

int LuaScriptInterface::luaDoCreateNewDungeon(lua_State* L)
{
	int32_t parameters = lua_gettop(L);
	ScriptEnviroment* env = getScriptEnv();
	PositionEx pos;
	popPosition(L, pos);
	TileSet tileset(407, 790, Wallset());
	DungeonType type(std::string("plainTown"), tileset);
	Dungeon dungeon(1, 2, 6, 3, type, pos);
	return 1;
