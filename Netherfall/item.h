...
public:
	...
	void setMagAttr(int32_t type, int32_t weight)
	{
		if (type == 0)
			return;
		magAttrs->createAttribute(type, weight);
		setIntAttr(ATTR_ITEM_MAGATTR, 1);
	}
	uint32_t getMagAttr() const {return (uint32_t)getIntAttr(ATTR_ITEM_MAGATTR);}
	
	uint32_t getMagAttrWeight(uint32_t type) const {
		return magAttrs->getAttributeWeightByType(type);
	}
