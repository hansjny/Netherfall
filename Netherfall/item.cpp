...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch(attr){
		...
		case ATTR_MAGATTR: 
		{
			...
			if(!propStream.GET_UINT32(attrCount)){
				return ATTR_READ_ERROR;
			}
			for (uint32_t i = 0; i < attrCount; i++) {
				uint32_t type;
				uint32_t weight;
				if(!propStream.GET_UINT32(type)){
					return ATTR_READ_ERROR;
				}
				if(!propStream.GET_UINT32(weight)){
					return ATTR_READ_ERROR;
				}
				setMagAttr(type, weight);
			}
			break;
		}
...
bool Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ATTR_ITEM_MAGATTR)){
		propWriteStream.ADD_UINT8(ATTR_MAGATTR);
		int32_t magAttrCount = magAttrs->getAttributeCount();
		propWriteStream.ADD_UINT32(magAttrCount);
		for (int i = 0; i < magAttrCount; i++) {
			propWriteStream.ADD_UINT32(magAttrs->getTypeAtPos(i));
			propWriteStream.ADD_UINT32(magAttrs->getWeightAtPos(i));
		}
	}
...
{
	std::stringstream s;
	int32_t magic_attribute = 0;
	if (item){
		subType = item->getSubType();	
		magic_attribute = item->getMagAttr(); 
	}
	...
	if (magic_attribute != 0) {
		uint32_t type; 
		uint32_t weight;
		for (int i = 0; i < item->magAttrs->getAttributeCount(); i++) {
			type = item->magAttrs->getTypeAtPos(i);
			weight = item->magAttrs->getWeightAtPos(i);
			switch (type) {
				case MAGICATTR_FIREDMG:
					s << std::endl<< weight << "% chance to cast great fireball on attack.";
					break;
				case MAGICATTR_EARTHDMG:
					s << std::endl<< weight << "% chance to cast poison storm on attack.";
					break;
				case MAGICATTR_ENERGYDMG:
					s << std::endl<< weight << "% chance to cast energy wave on attack.";
					break;
